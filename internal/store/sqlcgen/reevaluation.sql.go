...
freq AS (
    ...
    SELECT a.normalized_query,
           sum(power(2, - GREATEST(0, EXTRACT(EPOCH FROM (a.last_seen - ql.submitted_at)))
                        / $2::double precision))::double precision AS decayed_freq
    FROM survivors a
    JOIN search.query_log ql ON ql.normalized_query = a.normalized_query
        AND ql.submitted_at >= $1
...
changed AS (
    SELECT qa.normalized_query,
           ...
           ((COALESCE(s.distinct_users, 0) >= $3::int) AND NOT qa.banned) AS want
    FROM search.query_aggregates qa
    LEFT JOIN survivors s ON s.normalized_query = qa.normalized_query
    LEFT JOIN freq f ON f.normalized_query = qa.normalized_query
    WHERE (((COALESCE(s.distinct_users, 0) >= $3::int) AND NOT qa.banned)
           IS DISTINCT FROM qa.suggestible
...
func (q *Queries) ReevaluateSuggestible(ctx context.Context, arg ReevaluateSuggestibleParams) (int64, error) {
	result, err := q.db.Exec(ctx, reevaluateSuggestible,
		arg.WindowStart,
		arg.HalfLifeSeconds,
		arg.MinUsers,
		arg.Lim,
